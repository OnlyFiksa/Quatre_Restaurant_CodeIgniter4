...
<?= $this->section('title') ?>
<?= $mode == 'tambah' ? 'Tambah Menu Baru' : 'Edit Data Menu' ?>
<?= $this->endSection() ?>
...
<div class="bg-white p-6 rounded-2xl shadow-sm border border-sage-border">
    ...
    <div class="mb-8 border-b border-sage-border pb-4">
        <h3 class="text-xl font-bold text-sage-text">
            <?= $mode == 'tambah' ? '✨ Tambah Menu Baru' : '✏️ Edit Menu' ?>
        </h3>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="nama_menu" value="<?= $menu['nama_menu'] ?? '' ?>" required placeholder="Contoh: Nasi Goreng Spesial"
                           class="w-full p-3 bg-gray-50 border border-sage-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/50 focus:bg-white text-sage-text transition-all placeholder-gray-400">
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="relative">
                            <select name="id_kategori" required class="w-full p-3 bg-gray-50 border border-sage-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/50 focus:bg-white text-sage-text transition-all appearance-none cursor-pointer">
                                <option value="">-- Pilih Kategori --</option>
                                <option value="kat001" <?= (isset($menu) && $menu['id_kategori'] == 'kat001') ? 'selected' : '' ?>>🍽️ Makanan (kat001)</option>
                ...
                <div class="bg-sage-soft/30 p-5 rounded-2xl border border-sage-border">
                    ...
                    <div class="flex flex-col gap-3">
                        <label class="flex items-center gap-3 p-3 bg-white border border-sage-border rounded-xl cursor-pointer hover:border-primary transition-colors">
                            <input type="radio" name="status_menu" value="tersedia" class="accent-primary w-4 h-4" <?= (isset($menu) && $menu['status_menu'] == 'tersedia') ? 'checked' : '' ?>>
                        ...
                        <label class="flex items-center gap-3 p-3 bg-white border border-sage-border rounded-xl cursor-pointer hover:border-red-400 transition-colors">
                            <input type="radio" name="status_menu" value="habis" class="accent-red-500 w-4 h-4" <?= (isset($menu) && $menu['status_menu'] == 'habis') ? 'checked' : '' ?>>
