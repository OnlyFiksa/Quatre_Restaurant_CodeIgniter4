...
<head>
    ...
    <script>
        tailwind.config = {
            ...
            theme: {
                extend: {
                    colors: {
                        // Palette Sage Green Estetik (Kalem & Elegan)
                        "primary": "#558b6e",       /* Hijau Sage Utama */
                        "primary-hover": "#3d6650", /* Hijau Sage Lebih Gelap */
                        
                        "background-light": "#f4f7f5", /* Putih Tulang / Off-white */
                        "background-dark": "#1b2a25",
                        
                        "sage-soft": "#e1ece6",     /* Hijau Pudar (Background Item) */
                        "sage-text": "#2f483a",     /* Teks Hijau Tua Gelap */
                        "sage-border": "#cbdcd3"    /* Border Hijau Pudar */
                    },
                    fontFamily: {
    ...
    <style>
        ...
        /* FIX: Warna Menu Aktif sekarang mengikuti Sage Green (#558b6e) */
        .nav-item-active {
            background-color: #558b6e !important; /* Warna Sage */
            color: white !important;
            box-shadow: 0 4px 15px rgba(85, 139, 110, 0.3); /* Bayangan Sage Halus */
        }
        .nav-item-active span { color: white !important; }
        
        /* Hover Effect untuk menu non-aktif */
        .nav-item-hover:hover {
            background-color: #e1ece6; /* sage-soft */
            color: #2f483a; /* sage-text */
        }
    </style>
</head>
<body class="bg-background-light text-sage-text h-screen overflow-hidden flex">
    ...
    <aside class="w-72 bg-white border-r border-sage-border flex flex-col transition-all duration-300 z-20 shadow-sm">
        ...
        <div class="p-4 border-t border-sage-border">
            <button id="logout-btn" class="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-red-500 hover:bg-red-50 transition-all group">
    ...
    <main class="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <header class="h-20 bg-white/80 backdrop-blur-md border-b border-sage-border flex items-center justify-end px-8 sticky top-0 z-10">
        ...
        <div class="flex-1 overflow-y-auto p-8">
            <?php if (session()->getFlashdata('sukses')) : ?>
                <div class="mb-6 p-4 rounded-xl bg-primary/10 border border-primary/20 text-primary flex items-center gap-3 shadow-sm flash-msg">
                    <span class="material-symbols-outlined">check_circle</span>
                    <span class="font-medium"><?= session()->getFlashdata('sukses') ?></span>
                </div>
            <?php endif; ?>
            <?php if (session()->getFlashdata('error')) : ?>
                <div class="mb-6 p-4 rounded-xl bg-red-50 border border-red-200 text-red-600 flex items-center gap-3 shadow-sm flash-msg">
            ...
            <footer class="mt-12 pt-6 border-t border-sage-border text-xs text-sage-text/40 font-medium">
                <p>© 2026 Quatre Restaurant Admin.</p>
            </footer>
        </div>
    </main>

    <div id="logout-popup" class="fixed inset-0 z-50 hidden items-center justify-center bg-sage-text/40 backdrop-blur-sm transition-all opacity-0">
        <div class="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-sm text-center transform scale-95 transition-all border border-sage-border">
            <div class="size-14 bg-red-50 text-red-500 rounded-full flex items-center justify-center mx-auto mb-4">
                <span class="material-symbols-outlined text-3xl">logout</span>
            </div>
            <h2 class="text-xl font-bold text-sage-text mb-2">Konfirmasi Logout</h2>
            <p class="text-sage-text/60 mb-8 text-sm">Yakin ingin keluar aplikasi?</p>
            <div class="flex gap-3 justify-center">
                <button id="cancel-logout-btn" class="px-5 py-2.5 rounded-xl bg-background-light text-sage-text font-semibold hover:bg-sage-soft border border-sage-border transition-all">Batal</button>
